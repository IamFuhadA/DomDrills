<x-layouts.admin>
    <x-slot name="title">Edit Session — {{ $session->title }}</x-slot>
    <x-slot name="pageTitle">Edit Live Session</x-slot>
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('admin.sessions.update', $session) }}" class="space-y-4">
                ...
                <div class="form-group">
                    ...
                    <input id="title" name="title" type="text" class="form-input @error('title') border-state-error @enderror" value="{{ old('title', $session->title) }}" required>
                    @error('title')<p class="form-error">{{ $message }}</p>@enderror
                </div>

                <div class="form-group">
                    <label for="description" class="form-label">Description Summary</label>
                    <textarea id="description" name="description" class="form-textarea" rows="4" required>{{ old('description', $session->description) }}</textarea>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="form-group">
                        <label for="scheduled_at" class="form-label">Scheduled Time (IST)</label>
                        <input id="scheduled_at" name="scheduled_at" type="datetime-local" class="form-input" value="{{ old('scheduled_at', $session->scheduled_at->format('Y-m-d\TH:i')) }}" required>
                    </div>

                    <div class="form-group">
                        <label for="duration_minutes" class="form-label">Duration (Minutes)</label>
                        <input id="duration_minutes" name="duration_minutes" type="number" class="form-input" value="{{ old('duration_minutes', $session->duration_minutes) }}" required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="form-group">
                        ...
                        <input id="meeting_link" name="meeting_link" type="url" class="form-input" placeholder="https://zoom.us/j/..." value="{{ old('meeting_link', $session->meeting_link) }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="status" name="status" class="form-input">
                            ...
                            <option value="live" {{ $session->status === 'live' ? 'selected' : '' }}>Live Now</option>
